...
In this case the name is `A`, the constraint is `Any` and the reference capability is `val`. `Any` is used to mean that the type can be any type - it is not constrained. The remained of the class definition replaces `U32` with the type name `A`.
...
That tells the compiler what specific class to create, replacing `A` with the type provided. A `Foo[String]` is becomes equivalent to:
